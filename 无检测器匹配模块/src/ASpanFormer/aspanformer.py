...
from .utils.Roposition_Encoding import RoPEPositionEncodingSine
...
class ASpanFormer(nn.Module):
    def __init__(self, config):
        ...
        ##add
        self.pos_encoding1 = RoPEPositionEncodingSine(config['coarse']['d_model'])
    ...
    def forward(self, data, online_resize=False):
        ...
        feat_c0 = self.pos_encoding1(feat_c0,data['pos_scale0'])
        feat_c1 = self.pos_encoding1(feat_c1,data['pos_scale1'])
        [_, pos_encoding0] = self.pos_encoding(feat_c0,data['pos_scale0'])
        [_, pos_encoding1] = self.pos_encoding(feat_c0,data['pos_scale1'])
        feat_c0 = rearrange(feat_c0, 'n c h w -> n c h w ')
        feat_c1 = rearrange(feat_c1, 'n c h w -> n c h w ')
