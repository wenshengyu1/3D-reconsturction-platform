...
class messageLayer_ini(nn.Module):
    '''2.加入位置编码 3.Global cross Attention ,update实现了将位置编码pos与特征feat相融合，并且实现了交叉注意力'''
    ##对应initialize中的Glable Cross Attention
    ...
    def update(self,f0,f1,pos1,mask0,mask1):
        ...
        Returns:
            f0_new: (N, d, h, w)
        """
        #update实现了将位置编码pos与特征feat相融合，并且实现了交叉注意力
...
class messageLayer_gla(nn.Module):
    ...
    def decode_flow(self,flow_feature,kshape):   #MLP生成flow maps
        ...
        flow=self.flow_decoder(flow_feature.view(bs,-1,h*w)).permute(0,2,1).view(bs,h,w,4)   #MLP生成flow maps
...
class flow_initializer(nn.Module):
    'downsample'
    '''
    1.输入两个特征图 (feat0 和 feat1) 以及位置编码 (pos0 和 pos1)，它们通常是从图像提取的特征。
    2.对特征图进行下采样 (F.avg_pool2d)，以降低计算复杂度，并在低分辨率下建立光流匹配。 #本步由messageLayer_ini完成实现
    3.利用 Transformer 层 (messageLayer_ini) 计算全局交叉注意力，让 feat0 和 feat1 学习全局匹配关系。
    4.使用 1x1 卷积 (decoupler) 进行特征解耦：
        其中 sub_feat 负责视觉特征，
        sub_flow_feature 负责光流特征。
    5.将低分辨率的特征图上采样，恢复到原始尺寸，并融合回原始特征图 (feat0 和 feat1)。
    6.最终返回增强后的特征图 (feat0、feat1) 和光流特征 (flow_feature0、flow_feature1)。
    '''
    ...
    def forward(self, feat0, feat1,pos0,pos1,mask0=None,mask1=None,ds0=[4,4],ds1=[4,4]):
        ...
        #将下采样之后的特征加入位置编码以及实现了Global Cross Attention

        # decouple flow and visual features解耦feat与flow
...
class LocalFeatureTransformer_Flow(nn.Module):       ##总模块，之前是方法
    ...
    def forward(self, feat0, feat1,pos0,pos1,mask0=None,mask1=None,ds0=[4,4],ds1=[4,4]):
        ...
        pos0,pos1=self.pos_transform(pos0),self.pos_transform(pos1) ##会将 pos0 的通道数从 d_model 转换为 d_flow
        ...
        for layer in self.layers:    #将initial完成之后的特征feat0.1以及flow0.1放入GLA模块中，进行若干次加强(默认参数4)
